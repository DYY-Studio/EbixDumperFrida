...
import { PlatformImpl, iOSNativeFuncs } from "./types.js"

let initialized = false;

let agent: Agent;
let nativeFuncs: iOSNativeFuncs;
let pointers: Map<string, NativePointer>;

export function createIosImpl(): PlatformImpl {
  return {
    init,
    dumpAll: () => dumpall(),
...
function init() {
    ...
    nativeFuncs = {
        ...
        BV_getFileSize: BV_getFileSize,
        BV_readFile: BV_readFile
    };

    const kSecClass = moduleSecurity.getExportByName("kSecClass").readPointer();
    pointers.set("kSecClass", kSecClass)
    const kSecClassGenericPassword = moduleSecurity.getExportByName("kSecClassGenericPassword").readPointer();
    pointers.set("kSecClassGenericPassword", kSecClassGenericPassword)
    const kSecAttrAccount = moduleSecurity.getExportByName("kSecAttrAccount").readPointer();
    pointers.set("kSecAttrAccount", kSecAttrAccount)
    const kSecAttrService = moduleSecurity.getExportByName("kSecAttrService").readPointer();
    pointers.set("kSecAttrService", kSecAttrService)
    const kSecReturnData = moduleSecurity.getExportByName("kSecReturnData").readPointer();
    pointers.set("kSecReturnData", kSecReturnData)
    const kSecMatchLimit = moduleSecurity.getExportByName("kSecMatchLimit").readPointer();
    pointers.set("kSecMatchLimit", kSecMatchLimit)
    const kSecMatchLimitOne = moduleSecurity.getExportByName("kSecMatchLimitOne").readPointer();
    pointers.set("kSecMatchLimitOne", kSecMatchLimitOne)
    const kCFBooleanTrue = moduleSecurity.getExportByName("kCFBooleanTrue").readPointer();
    pointers.set("kCFBooleanTrue", kCFBooleanTrue)
}

function searchKeychainValueForKey(key: string, service: string) {
    const query = ObjC.classes.NSMutableDictionary.alloc().init();
    query.setObject_forKey_(pointers.get("kSecClassGenericPassword"), pointers.get("kSecClass"));
    query.setObject_forKey_(ObjC.classes.NSString.stringWithString_(key), pointers.get("kSecAttrAccount"));
    query.setObject_forKey_(ObjC.classes.NSString.stringWithString_(service), pointers.get("kSecAttrService"));
    query.setObject_forKey_(pointers.get("kCFBooleanTrue"), pointers.get("kSecReturnData"));
    query.setObject_forKey_(pointers.get("kSecMatchLimitOne"), pointers.get("kSecMatchLimit"));

    const result = Memory.alloc(Process.pointerSize);
    const status = nativeFuncs.SecItemCopyMatching(query, result);
    if (status === 0) {
...
async function extractLvfFile(filePath: string, envId: ObjC.Object) {
    ...
    for (const fileName of fileList) {
        ...
        const sizeResult = nativeFuncs.BV_getFileSize(
        ...
        const result = nativeFuncs.BV_readFile(
