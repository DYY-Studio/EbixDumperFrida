...
AGENT_ROOT_PATH: str = str(Path(__file__).parent.resolve() / "script" / "agent")
...
class DecrypterApplication(ConsoleApplication):
    ...
    def _start(self) -> None:
        try:
            ...
            try:
                ...
                self._local_path = Path(self._tmpdir)
    ...
    def _perform_pull(self) -> None:
        ...
        try:
            assert self._script is not None
            self._script.exports_sync.dumpall()
        except Exception as e:
            error = e
    ...
    def _process_message(self, message: Mapping[Any, Any], data: bytes) -> None:
        ...
        if message_type == "send":
            ...
            if ptype == "stream":
                ...
                self._file_format = payload["fileFormat"]
                handled = True
            elif ptype == "directory":
                os.makedirs(Path(self._local_path) / payload["path"], exist_ok=True)
                os.chmod(Path(self._local_path) / payload["path"],
                         stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
                handled = True
            elif ptype == "save":
                assert (self._bundle_id and self._file_format) is not None

                print() # blank line
                self._update_status(f"Packaging into {self._file_format}...")

                output_name = (Path(os.getcwd()) / f'{getLegalPath(self._bundle_id)}').name
                shutil.make_archive(output_name, 'zip', self._tmpdir, verbose=True)
                # Rename the .zip file
                (output_path := Path(os.getcwd()) / "output" / (output_name + f".{self._file_format}")).parent.mkdir(parents=True, exist_ok=True)
                if output_path.exists():
                    output_path.unlink()
                shutil.move(output_name + ".zip", output_path)

                self._update_status(f"Saved to ./{output_path.relative_to(os.getcwd())}")

                shutil.rmtree(self._tmpdir)
                os.mkdir(self._tmpdir)
                os.chmod(self._tmpdir, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
